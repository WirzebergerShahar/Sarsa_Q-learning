...
class World:
    ...
    def sarsa(self,total_episodes,alpha,dr,gamma=0.9):
        Q=self.init_q()
        epsilon=1
        ...
        for episode in range(total_episodes):
            ...
            while True:
                ...
                state2, reward, done = self.step(action1+1)
                # Choosing the next action
                action2 = self.next_action(Q,epsilon,state2)
                # Learning the Q-value
                predict = Q[state1 - 1, action1]
    ...
    def Qlearning(self,total_episodes,alpha,gamma=0.9):
        ...
        for episode in range(total_episodes):
            ...
            while True:
                ...
                if done:
                    break
            epsilon = 1/(episode/(total_episodes/10)+1)
        return Q

    def plot_actionValues(self, Q):
        Q1 = np.zeros(self.nStates)
        ...
        for i in range(nCols):
            for j in range(nRows, 0, -1):
                if k+1 not in stateHoles + stateGoal:
                    if(Q1[k]==Q[k,0]):
                        color='red'
                    plt.text(i + 0.5, j, str(round(Q[k,0],3)), fontsize=8,color=color,
                                 horizontalalignment='center', verticalalignment='top')
                    color='black'
                    plt.plot([i, i+1], [j-1, j], 'k-', lw=1)
                    plt.plot([i+1, i ], [j-1, j ], 'k-', lw=1)
                    if (Q1[k]==Q[k,1]):
                        color = 'red'
                    plt.text(i + 1, j - 0.5, str(round(Q[k,1],3)), fontsize=8,color=color,
                    ...
                    if (Q1[k] == Q[k, 3]):
                        color = 'red'
                    plt.text(i, j - 0.5, str(round(Q[k,3],3)), fontsize=8,color=color,
                    ...
                    if (Q1[k] == Q[k, 2]):
                        color = 'red'
                    plt.text(i + 0.5, j - 1, str(round(Q[k,2],3)), fontsize=8,color=color,
                                 horizontalalignment='center', verticalalignment='bottom')
                    color = 'black'
                    plt.plot([i, i + 1], [j - 1, j], 'k-', lw=1)
                    plt.plot([i + 1, i], [j - 1, j], 'k-', lw=1)
                else:
                    (I, J) = np.unravel_index(k+1, shape=(nRows, nCols), order='F')
                    coord = [[J, nRows - I],
                             [J + 1, nRows - I],
                             [J + 1, nRows - I + 1],
                             [J, nRows - I + 1],
                    ...
                    xs, ys = zip(*coord)
                    plt.fill(xs, ys, "red")
                    if k==self.stateGoal[0]-1:
                        plt.fill(xs, ys, "green")
                k += 1
        plt.title('MDP gridworld', size=16)
        plt.axis("equal")
        plt.axis("off")
